...
function file_is_type(file){
    ...
    const file_type = get_Type[get_Type.length - 1].toLowerCase();

    let group;
    switch(file_type){
        case 'jpg': case 'jpeg': case 'png': case 'svg': case 'webp': case 'jpe': case 'gif': case 'ico':
            group = 'image';
            break;

        case 'txt': case 'json': case 'md': case 'html': case 'php': case 'js': case 'css': case 'csv': case 'vcard':
            group = 'text';
            break;
    }
    return group;
...
function _alert(info){
    const alert_box = easyHTML('div', 'smplf-alert-box', '', body);
    const load_icon = easyHTML('div', 'smplf-alert-box-loader', '', alert_box);
    const alert_content = easyHTMLi('span', 'smplf-alert-content', '', info, alert_box);
}

function _alert_dismiss(seconds){
    function dismiss(){
        const alert_box = document.getElementById('smplf-alert-box')
        body.removeChild(alert_box);
    }
    setTimeout(dismiss, seconds);
}
...
/**
 * Holds the text contents of the current selected file
 */
let file_contents;

//
let download_contents;
...
function get_dir(target){
    ...
    xhr.onload = () => {
        if(xhr.getResponseHeader('for') != 'download'){
            const response = JSON.parse(xhr.responseText);
            response_data = response;
            recieved_file_info = response_data.fileinfo;
            file_contents = response_data.fileinfo.contents;
        } else {
            download_contents = xhr.responseText;
        }
    }
    xhr.send(target);
}
...
const nav_paste = easyHTML('button', 'smplf-nav-paste', 'fa fa-paste', nav_btns_div, ()=>{
    _element.onclick = ()=>{
        ...
        if(!select_files.on){
            ...
            if(Object.keys(select_files.seleced_folders).length == 0 && Object.keys(select_files.seleced_items).length == 0){
                ...
                if(select_files.from != response_data.path){
                    ...
                    const _sure = confirm('Are you sure you want to ' + select_files.operation + ' the selected items?');
...
const nav_select = easyHTML('button', 'smplf-nav-select', 'fa fa-file-circle-check', nav_btns_div, ()=>{
    _element.onclick = ()=>{
        if(!select_files.on){
            ...
            move_file_data = undefined;
            // Reset select files operation so that the user can select a new operation
            select_files.operation = '';
            // Set the visual ques
...
const nav_help = easyHTML('button', 'smplf-nav-help', 'fa fa-question-circle', nav_btns_div);
...
const micro_view_close = easyHTML('button', 'micro-view-close', 'fa-solid fa-xmark', micro_view, ()=>{
    _element.onclick = ()=>{
        micro_view.style.display = 'none';
    }
});

const micro_view_body = easyHTML('div', 'micro-view-body', '', micro_view);
    ...
    _element.onclick = ()=>{
        if(open_file_display.disabled){
            ...
            if(_sure){
                ...
                const _send = new SendData('edit_file', open_file_name.innerHTML, _path);
                _send.advanced.edited_content = open_file_display.value;
                
                get_dir(JSON.stringify(_send));
            } else {
                open_file_display.disabled = true;
                open_file_edit_btn.innerHTML = 'Edit';
                alert('Edit canceled!')
...
function display(data){
    ...
    for(let x = 0; x < data.folders.length; x++){
        ...
        const folder_div = easyHTML('div', 'smplf-folder-div', '', folder_grid, ()=>{
            ...
            _element.oncontextmenu = (event)=>{
                ...
                // Render the options
                // Clear micro view
                micro_view_body.innerHTML = '';
                // Add options buttons
                // The options container div
                const folder_opt_body             = easyHTML('div', 'file-opt-body', '', micro_view_body)
                ...
                const folder_opt_rename           = easyHTML('div', 'mv-options-btn-div', '', folder_opt_body, ()=>{
                                                    _element.onclick = ()=>{
                                                        ...
                                                        const new_name = prompt('Enter new folder name.', _folder_name);
                                                        // Check if its empty
                                                        if(new_name !== null){
                                                            if(new_name !== ''){
                ...
                const folder_opt_compress         = easyHTML('div', 'mv-options-btn-div', '', folder_opt_body, ()=>{
                                                _element.onclick = ()=>{
                                                    const _sure = confirm('Compress "' + _folder_name + '"?');
                                                    if(_sure){
                                                        const send = JSON.stringify(new SendData('zip_dir', _folder_name, current_path));
                                                        accessData(send);
                                                    }
                                                }
                                            });
                const folder_opt_compress_icon    = easyHTML('span', 'mv-opt-icon', 'fa-solid fa-file-zipper', folder_opt_compress);
                const folder_opt_compress_name    = easyHTMLi('span', 'mv-opt-name', '', ' Compess', folder_opt_compress);
    ...
    for(let x = 0; x < data.files.length; x++){
        ...
        const folder_div = easyHTML('div', 'smplf-folder-div', '', folder_grid, ()=>{
            ...
            _element.onclick = ()=>{
                ...
                if(select_files.on){
                    ...
                    micro_view_body.innerHTML = '';
                    // Add options buttons
                    const file_opt_body             = easyHTML('div', 'file-opt-body', '', micro_view_body)
                    ...
                    const file_opt_open             = easyHTML('div', 'mv-options-btn-div', '', file_opt_body, ()=>{
                                                        _element.onclick = ()=>{
                                                            ...
                                                            if(file_is_type(file_name) == 'text'){
                                                                ...
                                                                open_file_display.innerHTML = '';
                                                                const send = JSON.stringify(new SendData('get_file_contents', file_name, current_path));
                                                                get_dir(send);
                                                                open_file_display.value = file_contents;
                                                                open_file_body.style.display = 'block';
                                                            } else if(file_is_type(file_name) == 'image'){
                                                                open_file_view_div.innerHTML = '';
                                                                const image_viewer = easyHTML('img', 'image-viewer', '', open_file_view_div, ()=>{
                                                                    // _element.src = current_path + file_name;
                                                                    // open_file_body.style.display = 'block';
                                                                    window.open(current_path + file_name)
                                                                })
                                                            }
                                                        }
                    });
                    ...
                    const file_opt_rename           = easyHTML('div', 'mv-options-btn-div', '', file_opt_body, ()=>{
                                                        _element.onclick = ()=>{
                                                            const new_name = prompt('Enter new file name. (Do not include the file extension)', file_name);
                    ...
                    const file_opt_download         = easyHTML('div', 'mv-options-btn-div', '', file_opt_body, ()=>{
                                                        _element.onclick = ()=>{
                                                            const send = JSON.stringify(new SendData('download', file_name, current_path));
                                                            get_dir(send);

                                                            let file = new Blob([download_contents]);
                                                            console.log(file);

                                                            if(file_is_type(file_name) == 'text'){

                                                                const link = easyHTML('a', '', '', file_opt_download, ()=>{
                                                                    _element.href = URL.createObjectURL(file, { type: "application/octet-stream" });
                                                                    _element.download = file_name;
                                                                    _element.click();
                                                                })

                                                            } else {
                                                                window.open(site_URL + '/' + current_path + file_name);
                                                            }
                                                            ...
                                                            //     // _element.download = file_name;
                                                            //     // _element.click();
                                                            //     _element.href = site_URL + '/' + current_path + file_name;
                                                            //     // _element.download = file_name;
                                                            //     _element.click();
                                                            // })
                                                        }
                    ...
                    if(file_type == 'ZIP'){
                        ...
                        const file_opt_extract      = easyHTML('div', 'mv-options-btn-div', '', file_opt_body, ()=>{
                                                        _element.onclick = ()=>{
                                                            const _sure = confirm('Extract "' + file_name + '" here?');
                                                            if(_sure){
                                                                const send = JSON.stringify(new SendData('extract_zip', file_name, current_path));
                                                                accessData(send);
                                                            }
                                                        }
                                                    });
                        const file_opt_extract_icon = easyHTML('span', 'mv-opt-icon', 'fa-solid fa-file-zipper', file_opt_extract);
                        const file_opt_extract_name = easyHTMLi('span', 'mv-opt-name', '', ' Extract', file_opt_extract);
                    ...
                    // Info box
                    const info_box              = easyHTML('div', 'mc-info-box', '', micro_view_body);
        ...
        let file_icon
        if(file_is_type(file_name) == 'image'){
            file_icon = easyHTML('img', 'smplf-folder-icon', 'smplf-img-icon', folder_div, ()=>{
                _element.src = current_path + file_name;
            });
        } else {
            file_icon = easyHTML('span', 'smplf-folder-icon', 'smplf-file-icon icon-gray fa-solid ' + file_type_icon, folder_div);
        }
...
document.addEventListener('load',
    show_hide('smplf-nav-new-file', 'smplf-micro-view', 'block', ()=>{
        micro_view_body.innerHTML = '';
        const group = easyHTML('div', 'smplf-new-file-div', 'form-group', micro_view_body)
        ...
        const btn = easyHTML('button', 'smplf-mk-file', 'btn btn-primary', micro_view_body, ()=>{
...
document.addEventListener('load',
    show_hide('smplf-nav-new-folder', 'smplf-micro-view', 'block', ()=>{
        micro_view_body.innerHTML = '';
        const group = easyHTML('div', 'smplf-new-folder-div', 'form-group', micro_view_body)
        ...
        const btn = easyHTML('button', 'smplf-mk-dir', 'btn btn-primary', micro_view_body, ()=>{
            _element.innerHTML = 'Create';
            _element.onclick = ()=>{
                if(input.value == ''){
...
document.addEventListener('load',
    show_hide('smplf-nav-upload', 'smplf-micro-view', 'block', ()=>{
        micro_view_body.innerHTML = '';
        const group = easyHTML('form', 'smplf-upload-div', 'form-group', micro_view_body, ()=>{
            _element.onsubmit = (e)=>{
                e.preventDefault();
...
document.addEventListener('load',
    show_hide('smplf-nav-help', 'smplf-micro-view', 'block', ()=>{
        micro_view_body.innerHTML = '';
        const group = easyHTML('div', 'smplf-nav-help-div', 'form-group', micro_view_body);
        let heading = easyHTMLi('span', 'smplf-nav-help-heading', '', '<strong>Shortcuts<strong><br>', group);
        let text = easyHTMLi('span', 'smplf-nav-help-text', '', '1. <strong>S</strong> to enable and disable selection<br>', group);

        text = easyHTMLi('span', 'smplf-new-folder-input', '', '2. <strong>C</strong> to copy selected files<br>', group);
        text = easyHTMLi('span', 'smplf-new-folder-input', '', '3. <strong>X</strong> to cut selected files<br>', group);
        text = easyHTMLi('span', 'smplf-new-folder-input', '', '4. <strong>V</strong> to paste selected files<br>', group);
        text = easyHTMLi('span', 'smplf-new-folder-input', '', '5. <strong>Delete & Backspace</strong> to delete selected files<br><br>', group)

        heading = easyHTMLi('span', 'smplf-nav-help-heading', '', '<strong>Bug Reports<strong><br>', group);
        text = easyHTMLi('span', 'smplf-new-folder-input', '', 'For bug reports email us at <strong><a href="mailto://simpleline@proton.me">simpleline@proton.me</a></strong><br><br>', group);

        heading = easyHTMLi('span', 'smplf-nav-help-heading', '', '<strong>Get in touch<strong><br>', group);
        text = easyHTMLi('span', 'smplf-new-folder-input', '', '<strong>Email: </strong><a href="mailto://simpleline@proton.me">simpleline@proton.me</a><br>', group);
        text = easyHTMLi('span', 'smplf-new-folder-input', '', '<strong>Github: </strong><a href="https://github.com/victorkhalemberk/WP-simple-file-manager">WP Simple File Manager</a><br>', group);
    })
)
