...
class SimpleFiles {
    ...
    /**
     * Copies a directory and its files and and sub-directories to another location.
     * 
     * @param string $source the path to the directory to be copied.
     * @param string $destination the path to the directory in which to palce the copeid directory.
     */
    ...
    private function get_file_contents(){
        $contents = file_get_contents($this->path_and_item());
    ...
    private function edit_file($content){
        file_put_contents($this->path_and_item(), $content);
    }
    ...
    private function download_file(){
        ...
        header( "Content-disposition: attachment; filename=" . self::$item);
        header("for: download");
        readfile($this->path_and_item());
    }
    ...
    private function extract_zip(){
        ...
        $zip = new ZipArchive();
        $res = $zip->open($this->path_and_item());
        if($res == true){
    ...
    private function zip_dir(){

        $source = self::path_and_item();
        $destination = self::path_and_item();
    ...
    public function method_manager($root, $data, $uploads, $upl_data = 0){
        ...
        // Compression related managment

            case 'extract_zip':
                $this->extract_zip();
                $this->open_dir('scan_dir');
                $this->response['response'] = 'Files Extracted';
                echo json_encode($this->response);
                break;
            ...
            case 'zip_dir':
                $this->zip_dir();
                $this->open_dir('scan_dir');
                $this->response['response'] = 'Folder Compressed';
                echo json_encode($this->response);
                break;
